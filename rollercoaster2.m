...
At3 = getAT(z3,Tz3,h0);
Gfrontback(201:300) = ((At3+(Tz3./Tm3)*g)/g);
...
Gfrontback(401:500) = ((At5+(Tz5./Tm5)*g)/g);
...
Gfrontback(601:700) = (At7/g);
Gupdown(601:700) = sqrt(an7.^2+g^2)/g;

%Transition
t8 = linspace(pi/2,2*pi/3,100);
...
Gfrontback(701:800) = (At8+(Tz8./Tm8)*g)/g;
Gupdown(701:800) = -(an8+nz8*g)/g;


%Downward slope

t9 = linspace(0,20,100);
x9 = -cos(pi/6).*t9 + x8(end);
y9 = 0.*t9 + y8(end);
z9 = -sin(pi/6)*t9 + z8(end);
s9 = sqrt((x(end)-x(1))^2+(z(end)-z(1))^2);

N9 = cos(pi/6)*m*g*ones(1,length(t9));
Normx9 = sin(pi/6).*N9;
Normy9 = 0.*t9;
Normz9 = cos(pi/6).*N9;

Gfrontback(801:900) = (N9/g);
Gupdown(801:900) = sin(pi/6);


%Transition
rf = z9(end)/(1-cos(pi/6));
t10 = linspace(pi/3,pi/2,100);
x10 = rf*cos(t10) - sin(pi/6)*rf + x9(end);
y10 = 0.*t10 + y9(end);
z10 = -rf*sin(t10) + cos(pi/6)*rf + z9(end);
s10 = pi/6 * rf;
...
At10 = getAT(z10,Tz10,h0);

Gfrontback(901:1000) = (At10+(Tz10./Tm10)*g)/g;
...
function at = getAT(h,hprime,h0)
    at = (-2*9.8*hprime/2).*(1./sqrt(2*9.8*(h0-h)));
end
